...
@ActiveProfiles("dev")
@WithMockUser(authorities = "system:message:list")
public class AiMessageControllerTest {
    ...
    public void testGetMessageList() throws Exception {
        ...
        mockMvc.perform(get("/vibetempt/message/list"))
    ...
    public void testGetMessageInfo() throws Exception {
        when(permissionService.hasPermi(org.mockito.ArgumentMatchers.anyString())).thenReturn(true);
        Long id = 1L;
        mockMvc.perform(get("/vibetempt/message/{id}", id))
                .andExpect(status().isOk());
    }
} 