...
@ActiveProfiles("dev")
@WithMockUser(authorities = "system:session:list")
public class AiChatSessionControllerTest {
    ...
    public void testGetSessionList() throws Exception {
        ...
        mockMvc.perform(get("/vibetempt/session/list"))
    ...
    public void testGetSessionInfo() throws Exception {
        when(permissionService.hasPermi(org.mockito.ArgumentMatchers.anyString())).thenReturn(true);
        Long testId = 1L;
        mockMvc.perform(get("/vibetempt/session/{id}", testId))
                .andExpect(status().isOk());
    }
} 